...
import { JSBI, Token, TokenAmount, WETH, Fraction, Percent, CurrencyAmount } from '@titanswap-libs/sdk'
  ...
  return (
    <AutoColumn gap="20px">
      ...
      <DarkGray style={{ textAlign: 'center' }}>
        {t('swap112', {symbol: chainId && token.equals(WETH[chainId]) ? 'WETH' : token.symbol})}
      </DarkGray>
    </AutoColumn>
  )
}
    ...
    <BodyWrapper>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <MediumHeader>{t('swap114')}</MediumHeader>
          <div>
        ...
        {!account ? (
          <LargeHeader>{t('swap108')}</LargeHeader>
