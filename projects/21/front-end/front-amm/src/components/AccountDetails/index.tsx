...
import { Button } from '@titanswap-libs/uikit'
...
export default function AccountDetails({
  ...
  function formatConnectorName() {
    ...
    return <WalletName>{t('swap47')} {name}</WalletName>
  }
                  ...
                  <>
                    <AccountControl>
                      <div>
                        {account && (
                          <Copy toCopy={account}>
                            <span style={{ marginLeft: '4px' }}>{t('swap46')} </span>
                          </Copy>
                        )}
                        {chainId && account && (
                          <AddressLink
                          ...
                          >
                            <LinkIcon size={16} />
                            <span style={{ marginLeft: '4px' }}>{t('swap45')} </span>
                          </AddressLink>
                        )}
                      </div>
                    </AccountControl>
      ...
      {!!pendingTransactions.length || !!confirmedTransactions.length ? (
        <LowerSection>
          <AutoRow mb="1rem" style={{ justifyContent: 'space-between' }}>
            <Body>{t('swap13')}</Body>
            <LinkStyledButton onClick={clearAllTransactionsCallback}>({t('swap52')})</LinkStyledButton>
          </AutoRow>
        ...
        <LowerSection>
          <Body color={theme.colors.text}>{t('swap51')}</Body>
        </LowerSection>
      )}
    </>
  )
